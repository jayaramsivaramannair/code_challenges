'''
In a city-state of n people, 
there is a rumor going around that one of the n people is a spy for the neighboring city-state.
...
You are given a list of pairs, trust. 
Each trust[i] = [a, b] represents the fact that person a trusts person b.
...
but Person 3 does not trust either Person 1 or Person 2. Thus, Person 3 is the spy.
...
Since everyone trusts at least one other person, there is no spy.
...
Everyone trusts Person 3 but Person 3 does not trust anyone, so they are the spy.

source: codesignal.com (Lambda Final Sprint Challenge)
